 ...
 *         		         Transforms Into:
 * ==========================   V  ======================
for(range<int>::iterator temp = range(5,9).begin(); temp !=range(5,9).end(); ++temp)
					cout << *temp;
 * Imporant:
 * 1. begin() in y -> returns iterator
 * 2. end() in y -> returns iterator
 * 3. operator != in iterator
 * 4. opeartor * in iteartor
 * 5. prefix operator ++ in iteartor
 * 6. make template <typename T> in y
 */
...
int main() {
	...
	auto p = powerset(range(1,4)).begin();
	...
	for(auto p : powerset(zip(range('m','o'),range(15,17)))
		cout << p;
	return 0;
}
